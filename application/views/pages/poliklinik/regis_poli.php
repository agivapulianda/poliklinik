            ...
            <table id="example1" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example1_info">
              ...
              <?php 
               ...
               foreach ($regis_poli as $rgs_pl): ?>
                <tr>
                  <td><?php echo $no++ ?></td>
                  <td><?php echo $rgs_pl->nama_pasien?></td>
                  <td><?php echo $rgs_pl->hari?></td>
                  <td><?php echo $rgs_pl->tanggal?></td>
                  <td><?php echo $rgs_pl->status?></td>
                  <td onclick="javascript:return confirm('Anda yakin hapus ?')"><?php echo anchor('Poliklinik/hapus_regis/'.$rgs_pl->id_registrasi, '<div class="btn btn-danger btn-sm"><i class="fa fa-trash"></i></div>') ?></td>
                  <td><?php echo anchor('poliklinik/edit_regis/'.$rgs_pl->id_registrasi,'<div class="btn btn-primary btn-sm"><i class="fa fa-edit"></i></div>') ?></td>
                </tr>
              <?php endforeach ?>
            </table>
          </div>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form method="post" action="<?php echo base_url().'Poliklinik/tambah_aksi_regis'?>">
          <div class="form-grup">
            <label>Pasien</label>
            <select name="id_pasien" id="id_pasien" class="form-control">
                <option value="">Masukan Nama</option>
                <?php foreach ($M_regis_poli as $rgs_pl) : ?>
                  <option value="<?= $rgs_pl['id_pasien']; ?>"><?= $rgs_pl['nama_pasien']; ?> </option>
                <?php endforeach; ?>
            </select>
